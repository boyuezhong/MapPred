...
/* V1.02 */
/* modified by Qi Wu 2019 
covar matrices are converted into 231 matrices and a frobenius norm matrix
*/
...
int             main(int argc, char **argv)
{
    ...
    double sum, score, di, pdir, wtsum, idthresh=0.38, (** pab)[21][21], pa[MAXSEQLEN][21], **frob;
    float *weight, val, fval;
    char            buf[4096], name[512], seq[MAXSEQLEN], **aln;

    FILE *ifp, *pairofp, *frobofp;

    if (argc != 3 && argc != 4)
	fail("Usage: cov231stats [-p] alnfile 231channel_file frobenius_norm_file ");
	...
	fail("Cannot open 231stats file!");

    if (!(frobofp = fopen(argv[ar++], "wb")))
	fail("Cannot open frobenius file!");

    /* init frobenius norm */
    frob = allocmat(seqlen, seqlen, sizeof(double));
    for (i=0; i<seqlen; i++)
    {
	for (j=0; j<seqlen; j++)
	{
    ...
    /* convert covar matrices into 231 matrices*/
    for (a=0; a<21; a++)
    {
	for (b=0; b<a; b++)
	{
		...
		for (j=0; j<seqlen; j++)
		{
		    ...
		    fval = pab[i][j][b][a] - (pa[i][b] * pa[j][a]);
		    frob[i][j] += (val*val + fval*fval);
		    val += fval;
		    if (fwrite(&val, sizeof(float), 1, pairofp) != 1)
			fail("Write 231stats file failed!");
		}
	    }
	}
	b=a;
	...
	{
		val = sqrt(frob[i][j]);
		if (fwrite(&val, sizeof(float), 1, frobofp) != 1)
			fail("Write frobenius file failed!");
	}
    }
